...
(define (handle-mouse world x y button)
  (if (string=? button "button-down")
      (cond
        [(and (<= x (+ 40 (/ world-width 2))) (<= y (+ 416 (/ world-height 12))) (>= x 40) (>= y 416))
         (if (>= (elementoEnIndiceLista 2 world) (-(lengthList (car world)) 1))
             (append (eliminarIndiceLista 2 world)(cons
                                                   (string-append "Ganadores: " (~a (elementoLista (valorGanador (puntajes (car world))) (puntajes (car world)) 0))) '()))
          (append (eliminarIndiceLista 2 world)(cons (+ (elementoEnIndiceLista 2 world) 1) '())))]
        
        
        [(and (<= x (+ 40 (/ world-width 2))) (<= y (+ 482 (/ world-height 12))) (>= x 40) (>= y 482))
         (if (isInLista? 0 (elementoEnIndiceLista (elementoEnIndiceLista 2 world) (car world)) )
            (append (deal (append (car world) (cadr world))
               (cons (elementoEnIndiceLista 2 world) '()) 0 0 #t) (cons (elementoEnIndiceLista 2 world) '()))
          (append (deal (append (newColumna (car world)) (cadr world))
               (cons (elementoEnIndiceLista 2 world) '()) 0 0 #t) (cons (elementoEnIndiceLista 2 world) '())))]
...
(define (mesa height width todas_las_cartas)
  (place-image/align botones 0 400 "left" "top"
   (overlay/align "center" "top"
...
(define (draw-world todas_las_cartas)
  (overlay/align "right" "bottom" (text (~a (caddr todas_las_cartas)) 12 "black")(place-image
